...
$rutaCarpeta = "/administracion/reventa/img/";
...
foreach ($pictures as $imagen) {
    ...
    if (in_array($id_imagen, $array_imagenes)) {
        ...
        $new_name_image = $id_item . $k . "." . $extension;
        ...
        if (!empty($extension)) {
            ...
            if ($extension == "jpg" || $extension == "jpeg") {
                ...
                imagejpeg($fileImg, $ruta_final_img);
            } elseif ($extension == "png") {
                header('Content-Type: image/png');
                imagepng($fileImg, $ruta_final_img);
            }
            imagedestroy($fileImg);
            $imagen_server_oxas = "https://oxas.tech/administracion/reventa/img/$new_name_image";
...
if (isset($consulta->id)) {
    $id = $consulta->id;
    $sql = "INSERT INTO publicacion (user_id,CODIGO,NOMBRE,CODIGOORIGINAL,GANANCIA) VALUES ($id_user,'$id','$title','$id_item',$ganancia)";
