...
<body class="fixed-nav sticky-footer bg-dark" id="page-top">
  ...
  if(isset($_GET['search']) & !empty($_GET['search'])){
    ...
    curl_setopt($ch,CURLOPT_URL,'https://api.mercadolibre.com/users/'.$id_user.'/items/search?status=active&query='.$query.'&access_token='.$AccessToken);
  }else{
...
<div class="content-wrapper p-0">
    ...
    <div class="row align-items-center">
      ...
      <div class='col-auto pt-1 px-0 dropdown cursor-pointer' id='dropdown'>
        ...
        <div class='dropdown-menu'>
          <div class='ml-3 cursor-pointer hover-mouse' id='pausar'>Pausar</div>
          <!--div class="ml-3 cursor-pointer" id="edit_price">Modificar precios</div-->
      ...
      <div class='col-auto ml-4'>
        ...
        <small class='font-weight-bold' title='Publicaciones activas' data-toggle='tooltip'> (<?php echo $items_paginacion;?>)</small>
      </div>
      <div class='col-auto'>
        <a class='text-muted' href='pausadas.php' data-toggle='tooltip' title='Publicaciones Pausadas'>Pausadas</a>
      </div>
      <div class="col-auto">
      ...
      <div class="col-auto ml-auto">
        ...
        <!--a class="btn btn-link btn-sm text-success" href="nueva.php">Crear Publicaciones</a-->
      </div>
    </div>
  </div>
    ...
    if($total_items==0){
      ...
      if(isset($_GET['search']) & !empty($_GET['search'])){
        ...
        <div class='container mt-4 mb-5'>
          <strong class="mb-5">No tienes publicaciones activas con esta descripción: <span class="text-muted"><?php echo $_GET['search'];?></span></strong><br>
          <br><br><br>
        </div>
      <?php }else{ ?>
  ...
  <div class="modal fade images-<?php echo $id?>-modal-lg" tabindex="-1" role="dialog" aria-labelledby="myLargeModalLabel" aria-hidden="true">
    <div class="modal-dialog modal-lg">
      <div class="modal-content container">
        ...
        <div class="modal-body" id="container-images<?php echo $id?>"></div>
        <div class="modal-footer text-muted">
          Imágenes de la publicación.
          <div class="col-auto ml-auto">
            <button class="btn btn-primary btn-sm px-4" type="button" name="button" data-toggle="modal" data-target="#act_images<?php echo $id?>" id="openModalImagesRefresh<?php echo $id?>">Actualizar Imagenes</button>
          </div>
        </div>
      </div>
    </div>
  ...
  <div class="modal fade" id="act_images<?php echo $id?>" tabindex="-1" role="dialog" aria-labelledby="exampleModalLabel" aria-hidden="true">
    <div class="modal-dialog" role="document">
      <div class="modal-content">
        <div class="modal-body">
          <div class="row">
            <div class="col-sm-6">
              <h6 class="mb-3">Selecciona las imagenes de <strong>Tu publicación</strong> que deseas actualizar</h6>
              <div class="container-fluid" id="new_images<?php echo $id?>"></div>
            </div>
            <div class="col-sm-6">
              <h6 class="mb-3">Selecciona las imagenes de <strong>la publicación original</strong> que deseas agregar</h6>
              <div class="container-fluid" id="new_images_original<?php echo $id?>"></div>
            </div>
          </div>
        </div>
        <div class="modal-footer">
          <button type="button" class="btn btn-primary" id="button_act_img<?php echo $id?>">Actualizar</button>
        </div>
      </div>
    </div>
  </div>
  <script>
    //Al abrir modal Actualizar imagenes
    $(document).on('click',"#openModalImagesRefresh<?php echo $id?>",function(){
      $("#new_images_original<?php echo $id?>").empty();
      $.get('ajax/ajax_images.php',{item:'<?php echo $codigoOriginal?>'},verificar,'json');
      function verificar(respuesta){
        $.each(respuesta.pictures,function(i,resultado){
          $('#new_images_original<?php echo $id?>').append("<label class='categories'><img src="+resultado.secure_url+" width='50px'><input type='checkbox' value="+resultado.id+" name='imagesOriginales<?php echo $id?>'><span class='checkmark'></span></label>");
        });
      }
    });
    //Actualizar Imagenes del item
    $('#button_act_img<?php echo $id?>').click(function(){
      var checkboxes_propias=document.getElementsByName('images<?php echo $id?>');
      var array_imagenes_propias=[];var k=0;
      for(var i=0,n=checkboxes_propias.length;i<n;i++){
        if(checkboxes_propias[i].checked){
          array_imagenes_propias.push(checkboxes_propias[i].value);++k;
        }
      }
      var checkboxes_originales=document.getElementsByName('imagesOriginales<?php echo $id?>');
      var array_imagenes_originales=[];var k=0;
      for(var i=0,n=checkboxes_originales.length;i<n;i++){
        if(checkboxes_originales[i].checked){
          array_imagenes_originales.push(checkboxes_originales[i].value);++k;
        }
      }
      var string_imagenes_propias=array_imagenes_propias.join();
      var string_imagenes_originales=array_imagenes_originales.join();
      $.get('ajax/ajax_refresh_images.php',{item:'<?php echo $id?>',array_imagenes_propias:array_imagenes_propias,array_imagenes_originales:array_imagenes_originales},verificar,'text');
      function verificar(respuesta){
          const toast=swal.mixin({toast:true,position:'top-end',showConfirmButton:false,timer:3000});
          toast({type:'success',title:'¡Las imagenes fueron actualizadas exitosamente!'})
      }
    });
  </script>
  <script>
  //Imagenes del item
    $('#images<?php echo $id?>').click(function(){
      ...
      function verificar(respuesta){
        $.each(respuesta.pictures,function(i,resultado){
          ...
          $('#new_images<?php echo $id?>').append("<label class='categories'><img src="+resultado.secure_url+" width='50px'><input type='checkbox' value="+resultado.id+" name='images<?php echo $id?>'><span class='checkmark'></span></label>");
        });
      }
    });
    //Title
    $(document).on('click',"#titulo<?php echo $id?>",function(){
      ...
      if(typeof $("#actualizartitle<?php echo $id?>").val()!=='undefined'){
        $("#actualizartitle<?php echo $id?>").remove();$("#cancelar_title<?php echo $id?>").remove();
    ...
    $(document).on('click','#actualizartitle<?php echo $id?>',function(){
      var id="<?php echo $id?>", title=$("#titulo<?php echo $id?>").val();
  ...
  <div class="modal-dialog" role="document">
    <div class="modal-content">
      <div class="modal-header">
        ...
        <button class="close" type="button" data-dismiss="modal" aria-label="Close" id="close_quantity<?php echo $id;?>"><span aria-hidden="true">×</span></button>
      </div>
...
<script>
  ...
  $("#actualizarquantity<?php echo $id?>").click(function(){
    var id="<?php echo $id?>", quantity=$("#quantity_input<?php echo $id?>").val(), band=0, origin='active';
...
<div class="modal fade res-<?=$id?>-modal-lg" tabindex="-1" role="dialog" aria-labelledby="myLargeModalLabel" aria-hidden="true">
  <div class="modal-dialog modal-lg">
    <div class="modal-content container">
      <div class="modal-header">
        <div class="container">
          <div class="row"><h5 class="modal-title"><?php echo $title;?></h5></div>
        </div>
        <button class="close" type="button" data-dismiss="modal" aria-label="Close"><span aria-hidden="true">×</span></button>
      </div>
      <div class="modal-body text-muted container-fluid" id='body_cant_resp<?php echo $id?>'>
      </div>
      <div class="modal-footer text-muted"><small class="text-muted text-center">Se mostrarán un máximo de 20 respuestas automáticas.</small></div>
    </div>
  </div>
</div>
<script>
  ...
  $("#cant_respuestas<?=$id?>").click(function(){
    $("#body_cant_resp<?php echo $id?>").empty();var id="<?php echo $id?>";
    ...
    success: function(texto){
      ...
      if(texto!=0){
        ...
        $.each(respuestas,function(i,resultado){
          var auxiliar=resultado.split('~¬'), keywords=auxiliar[0].replace(/,/g,', ');
      ...
      }else{$("#body_cant_resp<?php echo $id?>").append("<div class='container-fluid text-success'>No tienes respuestas automáticas asociada a esta publicación.</div>")}
    }
  });
});
//Precio
$(document).on('click',"#price<?php echo $id?>",function(){
  ...
  if (typeof $("#actualizarprice<?php echo $id?>").val()!=='undefined'){
    $("#actualizarprice<?php echo $id?>").remove();$("#cancelar_price<?php echo $id?>").remove();
    $("#cont_precio<?php echo $id?>").append("<span class='auxiliar'>"+aux1+aux2+"</span>");
  }else{
    if(typeof $("span.auxiliar").val()!=='undefined'){
      $('span.auxiliar').each(function(){var aux=$(this).remove();});$("span.auxiliar").remove();
    }
    $("#cont_precio<?php echo $id?>").append("<span class='auxiliar'>"+aux1+aux2+"</span>");
  }
});
$(document).on('click','#cancelar_price<?php echo $id?>',function(){$('#actualizarprice<?php echo $id?>').remove();$('#cancelar_price<?php echo $id?>').remove();});
$(document).on('click','#actualizarprice<?php echo $id?>',function(){
  var id="<?php echo $id?>", price=$("#price<?php echo $id?>").val();
  ...
  <div class="modal-dialog modal-lg">
    <div class="modal-content container">
      <div class="modal-header">
        <div class="container">
          <div class="row"><h5 class="modal-title" id="exampleModalLabel"><?php echo $title;?></h5></div>
          <div class="row text-muted"><small id="sub<?php echo $id?>"></small></div>
        </div>
        <button class="close" type="button" data-dismiss="modal" aria-label="Close"><span aria-hidden="true">×</span></button>
      </div>
      <div class="modal-body"><div id="cont<?php echo $id?>" class="container"></div></div>
      <div class="modal-footer"><small class="text-muted">Se mostrarán máximo las últimas 50 preguntas que se han respondido en la actual publicación.</small></div>
    </div>
  </div>
</div>
<!-- Estadisticas -->
    ...
    <div class="modal-content container">
      ...
      <div class="modal-body container">
        <div class="row mb-3">
          ...
          <div class="col-auto"><button class="btn btn-success btn-sm" type="button" id='search_estadist<?php echo $id?>'>Buscar</button></div>
        </div>
        <div class="row"><canvas id="myAreaChart<?php echo $id?>" width="100%" height="30"></canvas></div>
      </div>
      <div class="modal-footer" id='chart_foot<?php echo $id?>'></div>
    </div>
  </div>
</div>
<script>
  $("#search_estadist<?php echo $id?>").click(function(){
    ...
    var mes=$("#mes<?php echo $id?>").val(), anio=$("#anio<?php echo $id?>").val(), item='<?php echo $id?>';
    ...
    function verificar(respuesta){
      ...
      window.graficaVisitas=new Chart(ctx,{
        ...
        options:{
          scales:{xAxes:[{time:{unit:'Días'},gridLines:{display:false},ticks:{maxTicksLimit: totaly}}],yAxes:[{ticks:{min:0,max: mayor,maxTicksLimit:8},gridLines:{color:"rgba(0, 0, 0, .125)",}}],},
...
<div class="modal fade op-<?php echo $id?>-modal-lg" tabindex="-2" role="dialog" aria-labelledby="myLargeModalLabel2" aria-hidden="true">
  <div class="modal-dialog modal-lg">
    <div class="modal-content container">
      <div class="modal-header">
        <div class="container">
          <div class="row"><h5 class="modal-title" id="exampleModalLabel2"><?php echo $title;?></h5></div>
        </div>
        <button class="close" type="button" data-dismiss="modal" aria-label="Close"><span aria-hidden="true">×</span></button>
      </div>
      <div class="modal-body">
        <div class="container">
          ...
          <div class="row"><div id="opti<?php echo $id?>"></div></div>
          <div class="row ml-3">
          ...
          <div class="row mr-2 mt-2">
            <b class="ml-2 col-auto">Pausar la publicación.</b>
            <button type="button" class="btn btn-sm btn-warning" id="pausar<?php echo $id?>">Pausar</button>
          </div>
          <div class="row"><div class="col-auto ml-3"><small class="text-muted">Ten en cuenta que los cambios tardarán un tiempo en reflejarse</small></div></div>
        </div>
      </div>
    </div>
  </div>
...
<div class="modal fade" id="trends<?php echo $id?>" tabindex="-4" role="dialog" aria-labelledby="exampleModalLabel" aria-hidden="true">
  <div class="modal-dialog" role="document">
    <div class="modal-content">
      ...
      <div class="modal-body text-muted" id="categ<?php echo $id?>"></div>
      <div class="modal-footer">
  ...
  <script>
    ...
    $(document).on('click','#pausar<?php echo $id?>',function(){
      var items="<?php echo $id?>", band=0;
      $.ajax({url:'ajax/ajax_pausar.php',type:'GET',data:{item:items,band:band},async:false,dataType:'json',
      success: function(json){
        ...
        if(typeof status!='undefined'){
          ...
          if(status=='paused'){
            const toast=swal.mixin({toast:true,position:'top-end',showConfirmButton:false,timer:3000});
            toast({type: 'success',title: '¡Fue pausada Exitosamente!'})
            $('#close_<?php echo $id?>').click();$("#pausar<?php echo $id?>").attr('disabled','disabled');
          }else{
            const toast=swal.mixin({toast:true,position:'top-end',showConfirmButton:false,timer:3000});
            toast({type:'error',title:"¡Hubo un problema! \n Inténtalo de nuevo."})
          }
  ...
  $("#but<?php echo $id?>").click(function(){
    $("#edit_description<?php echo $id?>").show();$("#edit_description_acept<?php echo $id?>").hide();
  ...
  $(document).on("click","#edit_description_acept<?php echo $id?>",function(){
    ...
    success: function(json){
      if(typeof json.last_updated!="undefined"){
        ...
        $("#edit_description_acept<?php echo $id?>").hide();$("#edit_description<?php echo $id?>").show();
        $("#textarea_description<?php echo $id?>").attr('disabled','disabled');$("#textarea_description<?php echo $id?>").css("background-color","#eee");
      }else{
        const toast=swal.mixin({toast:true,position:'top-end',showConfirmButton:false,timer:3000});
        toast({type:'error',title:"¡Hubo un problema! \n Inténtalo de nuevo."})
      }
...
$("#hist<?php echo $id?>").click(function(){
  ...
  $.get("ajax/ajax_historial.php",{item:item},verificar,'json');
  function verificar(respuesta){
...
$("#opt<?php echo $id?>").click(function(){
  ...
  var cat='<?php echo $categoria;?>', url='https://api.mercadolibre.com/categories/'+cat;
  $.getJSON(url,function(datos){
    var total=datos.total_items_in_this_category, ramas=datos.path_from_root, len=ramas.length;
...
$("#tendencia<?php echo $id?>").click(function(){
  ...
  var cat='<?php echo $categoria;?>', url='https://api.mercadolibre.com/sites/<?php echo $siteId;?>/trends/search?category='+cat;
  $.getJSON(url,function(datos){
    if(typeof datos!=='undefined' && datos.length>0){
      $.each(datos,function(i,resultado){
        var keyword=resultado.keyword, urlaux=resultado.url, aux="<div class='row px-3 mb-2'><b>"+(i+1)+"- </b><a href='"+urlaux+"' target='_blank'>"+keyword+'</a></div>';
        $("#categ<?php echo $id?>").append(aux);
      });
    }else{var opcionalt="<div class='text-success'>¡Esta rama no tiene tendencias!</div>";$("#categ<?php echo $id?>").append(opcionalt);}
  });
});
</script>
<?php } } } ?>
<div class="container mt-2">
  <div class="row justify-content-center">
    <?php if($items_paginacion>50){$NroPag=ceil($items_paginacion/50); ?>
      ...
      <script>
        ...
        $('#selectall').change(function(){
          ...
          $.get('../respuestas_automaticas/array_publicaciones.php',{all:true,band:band},verificar,'text');function verificar(respuesta){}
        });
        //apagar el checkbox general
        $(document).on('change','.public input[type=checkbox]',function(){if($(this).is(':checked')){}else{$("#selectall").prop('checked',false);}});
        ...
        $(document).on('click',"#publicaciones_activas input[name='publicaciones']",function(){
          var item=$(this).val();var band=0;if($(this).is(':checked')){band=1;}else{band=0;}
          $.get('../respuestas_automaticas/array_publicaciones.php',{item:item,band:band},verificar,'text');function verificar(respuesta){}
        });
        //pausar masivamente
        $(document).on('click','#pausar',function(){
          $("#loader_now").click();var band=1;
          $.get('ajax/ajax_pausar.php',{band:band},verificar,'text');
          function verificar(respuesta){
            if(respuesta!=''){
              publicaciones=respuesta.split(',');
              $('input:checkbox:checked').each(function(){
                var aux=$(this).val();var a=publicaciones.indexOf(aux);
                if(a!=(-1)){$(this).prop('checked',false);$(this).attr('disabled','disabled');$("#pausar"+aux).attr('disabled','disabled');$("#selectall").prop('checked',false);}
              });
              const toast=swal.mixin({toast:true,position:'top-end',showConfirmButton:false,timer:3000});
              toast({type:'success',title:'¡Fueron pausadas exitosamente!'})
              setTimeout ("$('button#close_loader').click();",500);
            }else{
              setTimeout ("$('button#close_loader').click();",500);
              const toast=swal.mixin({toast:true,position:'top-end',showConfirmButton:false,timer:3000});
              toast({type:'info',title:'¡No hay publicaciones para pausar!'})
            }
          }
        });
      </script>
      ...
      <div class="modal fade" id="modificar_stock" tabindex="-1" role="dialog" aria-labelledby="exampleModalLabel" aria-hidden="true">
        <div class="modal-dialog" role="document">
          <div class="modal-content">
            <div class="modal-header">
              <div class="container">
                <div class="row"><h5 class="modal-title">Modificar Stock</h5></div>
                <div class="row"><small class="text-muted" id='stock_a_modificar'></small></div>
              </div>
              <button class="close" type="button" data-dismiss="modal" aria-label="Close" id="close_stock"><span aria-hidden="true">×</span></button>
            </div>
            <div class="modal-body text-muted">
              <div class="container">
                <div class="row">
                  <select class="form-control col-3" id="condicion">
                    <option value="agregar">Agregar</option><option value="restar">Restar</option>
                  </select>
            ...
            <div class="modal-footer"><small class="text-muted">Ten en cuenta que puede tardar un momento hasta ver la información actualizada.</small></div>
          </div>
        </div>
      </div>
        ...
        <div class="modal-dialog" role="document">
          <div class="modal-content">
            <div class="modal-header">
              ...
              <button class="close" type="button" data-dismiss="modal" aria-label="Close" id='close_publicaciones'><span aria-hidden="true">×</span></button>
            </div>
            <div class="modal-body text-muted">
              <div class="container">
                <div class="row text-success ml-2 mb-2">Selecciona las publicaciones que deseas descargar.</div>
      ...
      <button type="button" hidden id='modal_masivas' data-toggle="modal" data-target=".modal-lg-masivas"></button>
      <!-- MAsivas -->
      <div class="modal fade modal-lg-masivas" tabindex="-2" role="dialog" aria-labelledby="myLargeModalLabel2" aria-hidden="true" data-backdrop="static" data-keyboard="false">
        <div class="modal-dialog modal-lg">
          <div class="modal-content container">
            <div class="modal-header">
              <div class="container"><div class="row"><h5 class="modal-title" id="exampleModalLabel2">Publicaciones Masivas</h5></div></div>
              <button class="close" type="button" data-dismiss="modal" aria-label="Close"><span aria-hidden="true">×</span></button>
            </div>
            <div class="modal-body">
              <div class="container">
                <span class="text-success">Se han publicado de manera exitosa <?php if(isset($_SESSION['masivas_exito'])){echo count($_SESSION['masivas_exito']);}?> publicaciones.</span>
                <span class="ml-auto text-primary cursor-pointer" id='show_table'>Ver</span>
                <div class="table-responsive mt-2" id='table_masivas'>
                  <table class="table table-striped">
                    ...
                    <tbody>
                      <?php $i=1;foreach($_SESSION['masivas_exito'] as $value){?>
                        <tr>
                          <?php $valores=explode('@',$value);?>
                          <th scope="row"><?php echo $i;?></th>
                          <td><?php echo $valores[0];?></td>
                          <td><?php echo $valores[1];?></td>
                          <td><?php echo $valores[2];?></td>
                          <td><?php $decimales=number_format($valores[3],2,'.',''); echo $decimales;?></td>
                        </tr>
                        <?php ++$i; } ?>
                      </tbody>
                    </table>
              ...
              <div class="modal-footer">
                <span class="text-muted"> <?php if(isset($_SESSION['masivas_fallo']) && count($_SESSION['masivas_fallo'])>0){ ?>
                  No se han podido publicar <?php echo count($_SESSION['masivas_fallo']); ?> publicaciones, revise los valores de cada item.
                  <button type="button" class="btn btn-link btn-sm" id='show_table_fallo'>revisar</button>
                <?php } ?>
              </span>
              <div class="table-responsive mt-2" id='table_masivas_fallo'>
                <table class="table table-striped">
                  <thead>
                    <tr>
                      <th scope="col">#</th>
                  ...
                  <tbody>
                    <?php $i=1;foreach($_SESSION['masivas_fallo'] as $value){?>
                      <tr>
                        <?php $valores=explode('@',$value);?>
                        <th scope="row"><?php echo $i;?></th>
                        <td><?php echo $valores[0];?></td>
        ...
        <script>
          $(document).on('click','#edit_stock',function(){var items=<?php echo $items_paginacion;?>;
          ...
          $(document).on('click','#actualizar_stock',function(){
            $("#close_stock").click();$("#loader_now").click();
            var stock=$("#stock").val(), condicion=$("#condicion").val(), band=1, origin='active';
            ...
            function verificar(respuesta){
              ...
              $.each(valores,function(i,resultado){
                ...
                $('input:checkbox:checked').each(function(){
                  var aux=$(this).val();
                  if(aux==auxiliar[0]){$(this).prop('checked',false);$("#stock_real"+auxiliar[0]).empty();$("#stock_real"+auxiliar[0]).append(auxiliar[1]);
                  if(auxiliar[1]==0){$("#stock_real"+auxiliar[0]).removeClass("text-success").addClass("text-danger");
                }else{$("#stock_real"+auxiliar[0]).removeClass("text-success").addClass("text-primary");}
              }
            });
          });
          const toast=swal.mixin({toast:true,position:'top-end',showConfirmButton:false,timer:3000});
    ...
    <script>
      $(document).ready(function(){
        //publicaicones masivas
        $('#table_masivas').hide();$('#table_masivas_fallo').hide();
        var aux=<?php if(isset($_GET['masive']) && $_GET['masive']==1){echo 1;}else{echo 0;}?>;
        if(aux>0){
          const toast=swal.mixin({toast:true,position:'top-end',showConfirmButton:false,timer:3000});
          toast({type:'success',title:'¡Fueron creadas exitosamente!'});
          $('#modal_masivas').click();
        }
        //publicaicon manual
        var aux2=<?php if(isset($_GET['create']) && $_GET['create']==1){echo 1;}elseif(isset($_GET['create']) && $_GET['create']==2){echo 0;}elseif (!isset($_GET['create'])){echo 3;}?>;
        if(aux2==1){
          const toast=swal.mixin({toast:true,position:'top-end',showConfirmButton:false,timer:3000});
          toast({type:'success',title:'¡Fue creada exitosamente!'})
        }else if(aux2==0){
          const toast=swal.mixin({toast:true,position:'top-end',showConfirmButton:false,timer:3000});
          toast({type:'error',title:'¡Hubo un error! \n Intentalo de nuevo.'})
        }
      });
      $(document).on('click','#show_table',function(){$('#table_masivas').show();});
      $(document).on('click','#show_table_fallo',function(){$('#table_masivas_fallo').show();});
    </script>
  </div>
...
<?php if(isset($_SESSION['masivas_exito']) && count($_SESSION['masivas_exito'])>0){$_SESSION['masivas_exito']=array();} ?>
<?php if(isset($_SESSION['masivas_fallo']) && count($_SESSION['masivas_fallo'])>0){$_SESSION['masivas_fallo']=array();} ?>
</body>
</html>
