...
$sql = "SELECT ACCESSTOKEN FROM usuario where id=$id_seller;";
$res = $conn->query($sql);
if ($res->num_rows > 0) {
...
{
  $sql = "SELECT REFRESTOKEN FROM usuario WHERE id=$id_seller;";
...
if (isset($_GET['orden_id'], $_GET['cedula'], $_GET['telefono'], $_GET['correo'], $_GET['b_emisor'], $_GET['b_receptor'], $_GET['pago'], $_GET['fecha_pago'], $_GET['referencia'], $_GET['agencia'], $_GET['estado'], $_GET['municipio'], $_GET['codigo_agencia'], $_GET['direccion'])) {
  $id_orden = $_GET['orden_id'];
  $cedula = $_GET['cedula'];
  $extension = $_GET['extension_cel'];
  $telefono = $_GET['telefono'];
  $telefono = $extension . $telefono;
  $correo = $_GET['correo'];
  $banco_emisor = $_GET['b_emisor'];
  $banco_receptor = $_GET['b_receptor'];
  $pago = $_GET['pago'];
